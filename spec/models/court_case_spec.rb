...
RSpec.describe CourtCase, type: :model do
  ...
  describe 'generated clean_case_number column' do
    context 'when case_number matches' do
      context 'when format ^[A-Za-z]{2}[0-9]{5,}' do
        it 'parses the case_number' do
          court_case = create(:court_case, case_number: 'CF94008641')
          court_case.reload
          expect(court_case.clean_case_number).to eq 'CF-1994-8641'
        end
      end

      context 'when format ^[A-Za-z]{2}-[0-9]{2}-[0-9]{1,}' do
        it 'parses the case_number' do
          court_case = create(:court_case, case_number: 'CF-94-8641')
          court_case.reload
          expect(court_case.clean_case_number).to eq 'CF-1994-8641'
        end
      end
      ...
      context 'when format ^[A-Za-z]{2}-[0-9]{4}-[0-9]{1,}' do
        it 'parses the case_number' do
          court_case = create(:court_case, case_number: 'CF-1994-8641')
          court_case.reload
          expect(court_case.clean_case_number).to eq 'CF-1994-8641'
        end
      end
    ...
    context 'when case_number does not match' do
      it 'is null' do
        court_case = create(:court_case, case_number: 'ABC-19991-0')
        court_case.reload
        expect(court_case.clean_case_number).to be nil
      end
    end
  end
