SELECT
  ...
  (CASE WHEN (
    ...
    FROM
      report_warrants
    WHERE
      parties.id = report_warrants.party_id
      AND report_warrants.code IN(
        'WAI$',
        'BWIFAP',
        'BWIFA',
        'BWIFC',
        ...
        WHERE
          parties.id = report_warrants.party_id
      	AND report_warrants.code IN('RETWA')
      ) THEN 'Yes' ELSE 'No' END) as has_active_warrant,
  ...
  (
    regexp_matches(
      split_part(counts.filed_statute_violation, 'O.S.', 1),
      '[0-9]{2}[A-Z]?'
    )
  ) [1] AS title_code
FROM
