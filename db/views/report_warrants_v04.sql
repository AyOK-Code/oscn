SELECT
	...
	ELSE
		FALSE
	END AS is_failure_to_appear, CASE WHEN docket_event_types.code IN('BWIFAP') THEN
		TRUE
	ELSE
		FALSE
	END AS is_failure_to_appear_and_pay, CASE WHEN docket_event_types.code IN('BWIFP') THEN
		TRUE
	ELSE
		FALSE
	...
	ELSE
		FALSE
	END AS is_arrest_warrant_issued, CASE WHEN docket_event_types.code IN('BWIAA') THEN
		TRUE
	ELSE
		FALSE
	END AS is_application_to_accelerate, CASE WHEN docket_event_types.code IN('BWIAR') THEN
		TRUE
	ELSE
		FALSE
	...
	END AS is_cleared, CAST(CAST((
		...
		SELECT
			(REGEXP_MATCHES(((
					SELECT
						(REGEXP_MATCHES(docket_events.description, 'WARRANT RETURNED \d{1,2}/\d{1,2}/\d{4}'))) [1]), '\d{1,2}/\d{1,2}/\d{4}')) [1]) AS warrant_returned_on, CASE WHEN (
			SELECT
				(REGEXP_MATCHES(((
						SELECT
							(REGEXP_MATCHES(docket_events.description, 'WARRANT ISSUED ON \d{1,2}/\d{1,2}/\d{4}'))) [1]), '\d{1,2}/\d{1,2}/\d{4}')) [1]) IS NULL THEN docket_events.event_on ELSE (
			SELECT
				(REGEXP_MATCHES(((
						SELECT
							(REGEXP_MATCHES(docket_events.description, 'WARRANT ISSUED ON \d{1,2}/\d{1,2}/\d{4}'))) [1]), '\d{1,2}/\d{1,2}/\d{4}')) [1])::DATE END AS warrant_issued_on, (
				SELECT
					(REGEXP_MATCHES(((
							SELECT
