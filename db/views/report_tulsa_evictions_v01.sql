SELECT
  ...
  (
    SELECT
      string_agg(distinct verdicts.name :: text, ', ')
    FROM
      issue_parties
      JOIN verdicts on verdicts.id = issue_parties.verdict_id
      JOIN parties on issue_parties.party_id = parties.id
  ...
  (
    ...
    WHERE
      docket_event_types_1.code :: text = 'A/' :: text
...
WHERE
  ...
  AND counties.name :: text = 'Tulsa' :: text